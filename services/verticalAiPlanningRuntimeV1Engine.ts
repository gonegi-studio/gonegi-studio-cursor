...
import {
  VERTICAL_AI_PLANNING_RUNTIME_V1_SEMVER,
  PLANNING_RUNTIME_COMPONENT_IDS,
  createPlanningRuntimeApi,
  createPlanningRuntimeRegistry,
  createPlanningCache,
  exportPlanningRuntime,
} from './verticalAiPlanningRuntimeV1.js';
import { loadCurrentGoalTruth } from './verticalAiDevelopmentV1Engine.js';
import { resolveProjectRoot } from './projectRootResolver.js';

export const VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE = 'PHASE-VAI-004' as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_SYSTEM_ID = 'VERTICAL_AI_PLANNING_RUNTIME_V1' as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_PASS_VERDICT = 'PASS_VERTICAL_AI_PLANNING_RUNTIME_V1' as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_FAIL_VERDICT = 'FAIL_VERTICAL_AI_PLANNING_RUNTIME_V1' as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_STATUS = 'VERTICAL_AI_PLANNING_RUNTIME_IMPLEMENTED' as const;

export const VERTICAL_AI_PLANNING_RUNTIME_V1_DIR = 'datasets/vertical_ai_planning_runtime_v1' as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_PATH =
  `${VERTICAL_AI_PLANNING_RUNTIME_V1_DIR}/vertical-ai-planning-runtime-v1.json` as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_EXPORT_PATH =
  `${VERTICAL_AI_PLANNING_RUNTIME_V1_DIR}/runtime-export-v1.json` as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_VERSION_PATH =
  `${VERTICAL_AI_PLANNING_RUNTIME_V1_DIR}/runtime-version-v1.json` as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_REGISTRY_PATH =
  `${VERTICAL_AI_PLANNING_RUNTIME_V1_DIR}/runtime-registry-v1.json` as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_FUNCTIONAL_EVIDENCE_PATH =
  `${VERTICAL_AI_PLANNING_RUNTIME_V1_DIR}/runtime-functional-evidence-v1.json` as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_REPRODUCIBILITY_PATH =
  `${VERTICAL_AI_PLANNING_RUNTIME_V1_DIR}/runtime-reproducibility-v1.json` as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_CONTRACTS_PATH =
  `${VERTICAL_AI_PLANNING_RUNTIME_V1_DIR}/runtime-contracts-v1.json` as const;
export const VERTICAL_AI_PLANNING_RUNTIME_V1_REPORT_PATH =
  'reports/vertical_ai/VERTICAL_AI_PLANNING_RUNTIME_V1_REPORT.json' as const;

const IMPLEMENTATION_FRAMEWORK_NAME =
  'Evidence-Bound Vertical AI Planning Runtime Implementation V1' as const;
const RUNTIME_ID = 'vertical_ai_planning_runtime' as const;

export const VAIPR_CONTRACT_IDS = [
  'VAIPR_FOUNDATION_VERIFIED',
  'VAIPR_COMPONENTS_IMPLEMENTED',
  'VAIPR_STABLE_RUNTIME_API',
  'VAIPR_MODULES_REPLACEABLE',
  'VAIPR_FUNCTIONAL_VERIFICATION',
  'VAIPR_REPRODUCIBLE',
  'VAIPR_DETERMINISTIC',
  'VAIPR_EVIDENCE_TO_PLAN_OPERATIONAL',
  'VAIPR_TRACEABLE_EXECUTION_PLANS',
  'VAIPR_REPOSITORY_FIRST',
  'VAIPR_EVIDENCE_FIRST',
  'VAIPR_REUSE_BEFORE_CREATE',
  'VAIPR_HUMAN_APPROVAL_BEFORE_WRITE',
  'VAIPR_READ_ONLY',
  'VAIPR_REFERENCE_ONLY',
  'VAIPR_NO_SOURCE_DATA_OWNERSHIP',
  'VAIPR_NO_PLATFORM_CORE_MUTATION',
  'VAIPR_NO_CIL_MUTATION',
  'VAIPR_NO_REPOSITORY_MUTATION',
  'VAIPR_FOUNDATION_UNCHANGED',
  'VAIPR_READY_FOR_EXECUTION_PLANNING',
  'VAIPR_RUNTIME_IMPLEMENTED',
] as const;

const REQUIRED_IMPLEMENTATION_COMPONENTS = [...PLANNING_RUNTIME_COMPONENT_IDS] as const;

const IMPLEMENTATION_PRINCIPLES = {
  implementation_only: true,
  follows_foundation: true,
  architecture_redesign: false,
  planning_runtime_is_only_public_interface: true,
  ...
  evidence_to_plan_operational: true,
...
function validateImplementationContracts(input: Record<string, boolean>) {
  const map: Array<[string, string]> = [
    ['VAIPR_FOUNDATION_VERIFIED', 'foundationVerified'],
    ['VAIPR_COMPONENTS_IMPLEMENTED', 'componentsImplemented'],
    ['VAIPR_STABLE_RUNTIME_API', 'stableRuntimeApi'],
    ['VAIPR_MODULES_REPLACEABLE', 'modulesReplaceable'],
    ['VAIPR_FUNCTIONAL_VERIFICATION', 'functionalVerification'],
    ['VAIPR_REPRODUCIBLE', 'reproducible'],
    ['VAIPR_DETERMINISTIC', 'deterministic'],
    ['VAIPR_EVIDENCE_TO_PLAN_OPERATIONAL', 'evidenceToPlanOperational'],
    ['VAIPR_TRACEABLE_EXECUTION_PLANS', 'traceableExecutionPlans'],
    ['VAIPR_REPOSITORY_FIRST', 'repositoryFirst'],
    ['VAIPR_EVIDENCE_FIRST', 'evidenceFirst'],
    ['VAIPR_REUSE_BEFORE_CREATE', 'reuseBeforeCreate'],
    ['VAIPR_HUMAN_APPROVAL_BEFORE_WRITE', 'humanApprovalBeforeWrite'],
    ['VAIPR_READ_ONLY', 'readOnly'],
    ['VAIPR_REFERENCE_ONLY', 'referenceOnly'],
    ['VAIPR_NO_SOURCE_DATA_OWNERSHIP', 'noSourceDataOwnership'],
    ['VAIPR_NO_PLATFORM_CORE_MUTATION', 'noPlatformCoreMutation'],
    ['VAIPR_NO_CIL_MUTATION', 'noCilMutation'],
    ['VAIPR_NO_REPOSITORY_MUTATION', 'noRepositoryMutation'],
    ['VAIPR_FOUNDATION_UNCHANGED', 'foundationUnchanged'],
    ['VAIPR_READY_FOR_EXECUTION_PLANNING', 'readyForExecutionPlanning'],
    ['VAIPR_RUNTIME_IMPLEMENTED', 'runtimeImplemented'],
  ];
  ...
  return {
    contract_ids: [...VAIPR_CONTRACT_IDS],
    results,
    aggregate_verdict: pass ? ('PASS' as const) : ('FAIL' as const),
  };
}

export function writeVerticalAiPlanningRuntimeV1EngineReport(): {
  passed: boolean;
  verdict: string;
  reportPath: string;
} {
  ...
  if (!foundationVerified) {
    issues.push({
      ...
      message: `Precheck requires ${VERTICAL_AI_PLANNING_FOUNDATION_V1_PASS_VERDICT}`,
      severity: 'error',
    });
  }
  ...
  const foundationManifest = readJson<{
    ...
    ready_for_planning_runtime?: boolean;
    design_fingerprint?: string;
  }>(root, VERTICAL_AI_PLANNING_FOUNDATION_V1_PATH);
  if (foundationManifest?.foundation_ready !== true) {
    issues.push({
      code: 'FOUNDATION_NOT_READY',
      message: 'Planning foundation must be ready before runtime implementation',
      severity: 'error',
    });
  }
  ...
  if (!allGoalsSatisfied) {
    issues.push({
      code: 'GOAL_TRUTH',
      message: 'All Current Goal Truth entries must be satisfied for planning runtime',
      severity: 'error',
    });
  }

  const api = createPlanningRuntimeApi(root);
  const registry = createPlanningRuntimeRegistry();
  const runtimeExport = exportPlanningRuntime(api, registry);
  const iface = api.describeInterface();

  const run1 = api.plan('vai004-smoke');
  const run2 = api.plan('vai004-smoke');
  const reproducible =
    run1.reproducible &&
    run2.reproducible &&
    run1.result_fingerprint === run2.result_fingerprint;
  const deterministic =
    run1.deterministic &&
    run1.execution_plan.plan_fingerprint === run2.execution_plan.plan_fingerprint;

  const writeGate = api.requestWrite('enact_execution_plan');
  ...
  const injectedApi = createPlanningRuntimeApi(root, {
    createCache: () => createPlanningCache(),
  });
  const injectedRun = injectedApi.plan('vai004-replaceability');
  const modulesReplaceable = injectedRun.result_fingerprint === run1.result_fingerprint;

  const evidenceToPlanOperational =
    run1.evidence.evidence_precedes_plan === true &&
    run1.traceability.evidence_to_plan === true &&
    run1.evidence.evidence_items.length > 0;

  const traceableExecutionPlans =
    run1.execution_plan.acyclic === true &&
    run1.execution_plan.steps.some((step) => step.action === 'await_approval') &&
    run1.validation.valid === true &&
    run1.traceability.chain.length > 0;
  ...
  const reuseBeforeCreate =
    ...
    run1.reuse.create_count === 0 &&
    run1.candidates.selected.every((candidate) => candidate.create_proposed === false);

  const functionalVerification =
    reproducible &&
    deterministic &&
    evidenceToPlanOperational &&
    traceableExecutionPlans &&
    repositoryFirst &&
    evidenceFirst &&
    reuseBeforeCreate &&
  ...
  if (!functionalVerification) {
    issues.push({
      ...
      message: 'Planning Runtime failed functional smoke checks',
      severity: 'error',
    });
  }
  ...
  const stableRuntimeApi =
    iface.version === VERTICAL_AI_PLANNING_RUNTIME_V1_SEMVER &&
    iface.public_surface === 'planning_runtime' &&
  ...
  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_FUNCTIONAL_EVIDENCE_PATH, {
    runtime_functional_evidence_v1_id: 'planning_runtime_functional_evidence_v1',
    architecture_version: PROJECT_BRAIN_ARCHITECTURE_VERSION,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    ...
    plan_fingerprint: run1.execution_plan.plan_fingerprint,
    plan_order: run1.execution_plan.order,
    understanding_fingerprint: run1.understanding_fingerprint,
    candidate_count: run1.candidates.candidates.length,
    selected_count: run1.candidates.selected.length,
    reuse_count: run1.reuse.reuse_count,
    gap_count: run1.gaps.gap_count,
    risk_count: run1.risks.risk_count,
    ...
    evidence_to_plan_operational: evidenceToPlanOperational,
    traceable_execution_plans: traceableExecutionPlans,
  ...
  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_REPRODUCIBILITY_PATH, {
    runtime_reproducibility_v1_id: 'planning_runtime_reproducibility_v1',
    architecture_version: PROJECT_BRAIN_ARCHITECTURE_VERSION,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    generated_at: generatedAt,
    reproducible,
  ...
  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_EXPORT_PATH, {
    ...runtimeExport,
    architecture_version: PROJECT_BRAIN_ARCHITECTURE_VERSION,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    generated_at: generatedAt,
  ...
  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_VERSION_PATH, {
    runtime_version_v1_id: 'planning_runtime_version_v1',
    architecture_version: PROJECT_BRAIN_ARCHITECTURE_VERSION,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    generated_at: generatedAt,
    runtime_version: VERTICAL_AI_PLANNING_RUNTIME_V1_SEMVER,
    consumes_foundation: VERTICAL_AI_PLANNING_FOUNDATION_V1_PATH,
  });

  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_REGISTRY_PATH, {
    ...registry,
    architecture_version: PROJECT_BRAIN_ARCHITECTURE_VERSION,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    generated_at: generatedAt,
    export_ref: VERTICAL_AI_PLANNING_RUNTIME_V1_EXPORT_PATH,
  ...
  const baselineChecks: Array<[string, { preserved: boolean; drift: string[] }, string]> = [
    ...
    ['FOUNDATION_BASELINE_MUTATION', foundationBaselineAfter, 'Planning foundation baseline changed'],
  ];
  ...
  const noRepositoryMutation =
    IMPLEMENTATION_PRINCIPLES.repository_mutation === false && writeGate.authorized === false;

  const designFingerprint = foundationManifest?.design_fingerprint ?? null;
  const implementationFingerprint = stableFingerprint({
    components: PLANNING_RUNTIME_COMPONENT_IDS,
  ...
  const decisionFingerprint = stableFingerprint({
    ...
    result: run1.result_fingerprint,
    plan: run1.execution_plan.plan_fingerprint,
    reproducible,
    deterministic,
  });

  const readyForExecutionPlanning =
    foundationVerified &&
    foundationManifest?.ready_for_planning_runtime === true &&
  ...
  if (contractValidation.aggregate_verdict !== 'PASS') {
    issues.push({
      ...
      message: 'One or more Planning Runtime contracts failed',
      severity: 'error',
    });
  }

  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_CONTRACTS_PATH, {
    runtime_contracts_v1_id: 'planning_runtime_contracts_v1',
    architecture_version: PROJECT_BRAIN_ARCHITECTURE_VERSION,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    generated_at: generatedAt,
    contract_ids: [...VAIPR_CONTRACT_IDS],
  ...
  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_PATH, {
    vertical_ai_planning_runtime_v1_id: 'vertical_ai_planning_runtime_v1',
    architecture_version: PROJECT_BRAIN_ARCHITECTURE_VERSION,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    system_id: VERTICAL_AI_PLANNING_RUNTIME_V1_SYSTEM_ID,
    generated_at: generatedAt,
    mode: 'READ_ONLY',
    ...
    ready_for_execution_planning: readyForExecutionPlanning,
    planning_reproducible: reproducible,
    deterministic,
    evidence_to_plan_operational: evidenceToPlanOperational,
    traceable_execution_plans: traceableExecutionPlans,
    components: [...PLANNING_RUNTIME_COMPONENT_IDS],
    foundation_ref: VERTICAL_AI_PLANNING_FOUNDATION_V1_PATH,
    export_ref: VERTICAL_AI_PLANNING_RUNTIME_V1_EXPORT_PATH,
    registry_ref: VERTICAL_AI_PLANNING_RUNTIME_V1_REGISTRY_PATH,
    version_ref: VERTICAL_AI_PLANNING_RUNTIME_V1_VERSION_PATH,
    functional_evidence_ref: VERTICAL_AI_PLANNING_RUNTIME_V1_FUNCTIONAL_EVIDENCE_PATH,
    reproducibility_ref: VERTICAL_AI_PLANNING_RUNTIME_V1_REPRODUCIBILITY_PATH,
    contracts_ref: VERTICAL_AI_PLANNING_RUNTIME_V1_CONTRACTS_PATH,
    principles: IMPLEMENTATION_PRINCIPLES,
    platform_core_mutation: false,
    cil_mutation: false,
    repository_mutation: false,
  ...
  const verdict = passed
    ...
    : VERTICAL_AI_PLANNING_RUNTIME_V1_FAIL_VERDICT;

  const report = {
    report_id: `vertical_ai_planning_runtime_v1_${Date.now()}`,
    phase: VERTICAL_AI_PLANNING_RUNTIME_V1_PHASE,
    system_id: VERTICAL_AI_PLANNING_RUNTIME_V1_SYSTEM_ID,
    generated_at: generatedAt,
    goal: 'Implement Vertical AI Planning Runtime as a read-only, evidence-first, repository-first, reuse-before-create planner with human approval before write.',
    vertical_ai_planning_runtime_v1_passed: passed,
    final_verdict: verdict,
    status: passed
      ? VERTICAL_AI_PLANNING_RUNTIME_V1_STATUS
      : 'VERTICAL_AI_PLANNING_RUNTIME_NOT_READY',
    ...
    deterministic,
    evidence_to_plan_operational: evidenceToPlanOperational,
    traceable_execution_plans_established: traceableExecutionPlans,
    ready_for_execution_planning: readyForExecutionPlanning,
  ...
  writeJson(root, VERTICAL_AI_PLANNING_RUNTIME_V1_REPORT_PATH, report);

  return {
    passed,
    verdict,
    reportPath: VERTICAL_AI_PLANNING_RUNTIME_V1_REPORT_PATH,
  };
}
