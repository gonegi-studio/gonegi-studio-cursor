...
export const IMAGE_APP_IMPORT_TEST_PHASE =
  'PHASE-SOURCE-VIDEO-070-MOVIE_ANALYSIS_IMAGE_APP_IMPORT_TEST_V1' as const;
export const IMAGE_APP_IMPORT_TEST_PASS_VERDICT =
  'PASS_MOVIE_ANALYSIS_IMAGE_APP_IMPORT_TEST_V1' as const;
export const IMAGE_APP_IMPORT_TEST_FAIL_VERDICT =
  'FAIL_MOVIE_ANALYSIS_IMAGE_APP_IMPORT_TEST_V1' as const;
export const IMAGE_APP_IMPORT_TEST_REPORT_PATH =
  'reports/movie-analysis-image-app-import-test-report.json' as const;
export const IMAGE_APP_IMPORT_TEST_MD_PATH =
  'reports/MOVIE_ANALYSIS_IMAGE_APP_IMPORT_TEST.md' as const;

export { EXPECTED_SOURCE_COUNT, EXPECTED_ADAPTER_COUNT, ADAPTERS_PER_SOURCE };

export type ValidationStatus = 'PASS' | 'FAIL';

export type ImageAppImportTestIssue = {
  code: string;
  message: string;
...
export type SourceImageAppImportAudit = {
  source_video_id: string;
...
export type MovieAnalysisImageAppImportTestReport = {
  report_id: string;
  phase: typeof IMAGE_APP_IMPORT_TEST_PHASE;
  timestamp: string;
  planning_only: true;
  generation: false;
  ...
  storytelling_adapter_import: ValidationStatus;
  traceability_preserved: ValidationStatus;
  image_app_import_ready: ValidationStatus;
  planning_only_status: ValidationStatus;
  source_audits: SourceImageAppImportAudit[];
  final_verdict:
    | typeof IMAGE_APP_IMPORT_TEST_PASS_VERDICT
    | typeof IMAGE_APP_IMPORT_TEST_FAIL_VERDICT;
  issues: ImageAppImportTestIssue[];
};
...
function simulateImageAppBridgeImport(entry: ImageAppBridgeEntry): boolean {
  ...
  return (
    ...
    mapping.character_pattern_refs.length > 0 &&
...
function auditSource(
  ...
  const bridgeImportable = simulateImageAppBridgeImport(entry);
  if (!bridgeImportable) {
...
function aggregateStatus(
  audits: SourceImageAppImportAudit[],
  field: keyof SourceImageAppImportAudit
): ValidationStatus {
...
function buildMarkdown(report: MovieAnalysisImageAppImportTestReport): string {
  const lines = [
    '# Movie Analysis Image App Import Test',
    '',
    `**Phase:** ${report.phase}`,
...
export function writeMovieAnalysisImageAppImportTestReport(
  projectRoot?: string
): MovieAnalysisImageAppImportTestReport {
  const root = resolveProjectRoot(projectRoot);
  const issues: ImageAppImportTestIssue[] = [];
  ...
  if (!bridge || !releasePackage || !adapterLibrary) {
    const report: MovieAnalysisImageAppImportTestReport = {
      report_id: 'movie-analysis-image-app-import-test-report-v1',
      ...
      storytelling_adapter_import: 'FAIL',
      traceability_preserved: 'FAIL',
      image_app_import_ready: 'FAIL',
      planning_only_status: 'FAIL',
      source_audits: [],
      final_verdict: IMAGE_APP_IMPORT_TEST_FAIL_VERDICT,
      issues,
    };

    fs.mkdirSync(path.join(root, 'reports'), { recursive: true });
    fs.writeFileSync(
      path.join(root, IMAGE_APP_IMPORT_TEST_REPORT_PATH),
      `${JSON.stringify(report, null, 2)}\n`,
      'utf8'
    );
    fs.writeFileSync(
      path.join(root, IMAGE_APP_IMPORT_TEST_MD_PATH),
  ...
  if (bridge.consumer_target !== 'image_app') {
    issues.push({
      code: 'BRIDGE_TARGET_INVALID',
      message: 'Image App bridge consumer_target must be image_app',
      severity: 'error',
    });
  }
  ...
  const sourceAudits: SourceImageAppImportAudit[] = [];
  ...
  for (const sourceVideoId of EXPECTED_SOURCE_VIDEO_IDS) {
    ...
    if (audit.import_ready === 'FAIL') {
      issues.push({
        ...
        message: `Image App import not ready for ${sourceVideoId}`,
        severity: 'error',
        source_video_id: sourceVideoId,
      });
    }
  ...
  const pass = imageAppImportReady === 'PASS';
  ...
  const report: MovieAnalysisImageAppImportTestReport = {
    ...
    final_verdict: pass
      ...
      : IMAGE_APP_IMPORT_TEST_FAIL_VERDICT,
    issues,
  };
