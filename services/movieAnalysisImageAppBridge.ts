...
export const IMAGE_APP_BRIDGE_PHASE =
  'PHASE-SOURCE-VIDEO-069-MOVIE_ANALYSIS_IMAGE_APP_BRIDGE_V1' as const;
export const IMAGE_APP_BRIDGE_PASS_VERDICT =
  'PASS_MOVIE_ANALYSIS_IMAGE_APP_BRIDGE_V1' as const;
export const IMAGE_APP_BRIDGE_FAIL_VERDICT =
  'FAIL_MOVIE_ANALYSIS_IMAGE_APP_BRIDGE_V1' as const;
export const IMAGE_APP_BRIDGE_DIR = 'exports/movie_analysis_image_bridge' as const;
export const IMAGE_APP_BRIDGE_PATH =
  'exports/movie_analysis_image_bridge/movie-analysis-image-app-bridge.json' as const;
export const IMAGE_APP_BRIDGE_REPORT_PATH =
  'reports/movie-analysis-image-app-bridge-report.json' as const;

export { EXPECTED_SOURCE_COUNT, EXPECTED_ADAPTER_COUNT, ADAPTERS_PER_SOURCE };

export type ValidationStatus = 'PASS' | 'FAIL';

export type ImageAppBridgeSafetySummary = {
  planning_only: true;
  generation: false;
...
export type ImageAppBridgeAdapterMapping = {
...
export type ImageAppBridgeEntry = {
  ...
  adapter_mapping: ImageAppBridgeAdapterMapping;
  image_app_ready: true;
  bridge_only: true;
};

export type MovieAnalysisImageAppBridge = {
  bridge_id: string;
  bridge_type: 'movie_analysis_image_app_bridge';
  phase: typeof IMAGE_APP_BRIDGE_PHASE;
  consumer_target: 'image_app';
  generated_at: string;
  ...
  entries: ImageAppBridgeEntry[];
  safety_summary: ImageAppBridgeSafetySummary;
};

export type ImageAppBridgeIssue = {
  code: string;
  message: string;
...
export type SourceImageAppBridgeAudit = {
  source_video_id: string;
...
export type MovieAnalysisImageAppBridgeReport = {
  report_id: string;
  phase: typeof IMAGE_APP_BRIDGE_PHASE;
  timestamp: string;
  planning_only: true;
  ...
  continuity_adapter_mapped: ValidationStatus;
  storytelling_adapter_mapped: ValidationStatus;
  adapter_mapping_complete: ValidationStatus;
  traceability_preserved: ValidationStatus;
  image_app_bridge_ready: ValidationStatus;
  planning_only_status: ValidationStatus;
  source_audits: SourceImageAppBridgeAudit[];
  final_verdict:
    | typeof IMAGE_APP_BRIDGE_PASS_VERDICT
    | typeof IMAGE_APP_BRIDGE_FAIL_VERDICT;
  issues: ImageAppBridgeIssue[];
};

const BRIDGE_SAFETY_SUMMARY: ImageAppBridgeSafetySummary = {
  planning_only: true,
  generation: false,
  runtime_execution: false,
  video_generation: false,
...
function buildAdapterMapping(libraryEntry: DnaAdapterLibraryEntry): ImageAppBridgeAdapterMapping {
  return {
    consumer_target: 'image_app',
    scene_adapter: true,
    camera_adapter: true,
...
function aggregateStatus(
  audits: SourceImageAppBridgeAudit[],
  field: keyof SourceImageAppBridgeAudit
): ValidationStatus {
...
export function buildMovieAnalysisImageAppBridge(
  releasePackage: MovieAnalysisDnaReleasePackage,
  adapterLibrary: NonNullable<ReturnType<typeof loadMovieAnalysisDnaAdapterLibrary>>
): MovieAnalysisImageAppBridge {
  const entries: ImageAppBridgeEntry[] = [];
  ...
  return {
    bridge_id: 'movie-analysis-image-app-bridge-v1',
    bridge_type: 'movie_analysis_image_app_bridge',
...
export function writeMovieAnalysisImageAppBridge(
  projectRoot?: string
): { bridge: MovieAnalysisImageAppBridge; report: MovieAnalysisImageAppBridgeReport } {
  const root = resolveProjectRoot(projectRoot);
  const issues: ImageAppBridgeIssue[] = [];
  ...
  if (!productionReadyReport || !releasePackage || !adapterLibrary) {
    const report: MovieAnalysisImageAppBridgeReport = {
      report_id: 'movie-analysis-image-app-bridge-report-v1',
      phase: IMAGE_APP_BRIDGE_PHASE,
      timestamp,
      planning_only: true,
      generation: false,
      ...
      traceability_preserved: 'FAIL',
      image_app_bridge_ready: 'FAIL',
      planning_only_status: 'FAIL',
      source_audits: [],
      final_verdict: IMAGE_APP_BRIDGE_FAIL_VERDICT,
      issues,
    };
    fs.mkdirSync(path.join(root, 'reports'), { recursive: true });
    fs.writeFileSync(
      path.join(root, IMAGE_APP_BRIDGE_REPORT_PATH),
      `${JSON.stringify(report, null, 2)}\n`,
      'utf8'
    );
    return { bridge: null as unknown as MovieAnalysisImageAppBridge, report };
  }
  ...
  if (releasePackage.release_readiness.release_ready !== true) {
    issues.push({
      ...
      message: 'DNA release package is not ready for Image App bridge',
      severity: 'error',
    });
  }
  ...
  const bridge = buildMovieAnalysisImageAppBridge(releasePackage, adapterLibrary);

  const outDir = path.join(root, IMAGE_APP_BRIDGE_DIR);
  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(
    path.join(root, IMAGE_APP_BRIDGE_PATH),
    `${JSON.stringify(bridge, null, 2)}\n`,
    'utf8'
  );

  const sourceAudits: SourceImageAppBridgeAudit[] = [];
  for (const sourceVideoId of EXPECTED_SOURCE_VIDEO_IDS) {
    ...
    if (audit.source_bridge_ready === 'FAIL') {
      issues.push({
        ...
        message: `Image App bridge not ready for ${sourceVideoId}`,
        severity: 'error',
        source_video_id: sourceVideoId,
      });
    }
  ...
  const pass = imageAppBridgeReady === 'PASS';
  ...
  const report: MovieAnalysisImageAppBridgeReport = {
    ...
    traceability_preserved: traceabilityPreserved,
    image_app_bridge_ready: imageAppBridgeReady,
    planning_only_status: planningOnlyStatus,
    source_audits: sourceAudits,
    final_verdict: pass ? IMAGE_APP_BRIDGE_PASS_VERDICT : IMAGE_APP_BRIDGE_FAIL_VERDICT,
    issues,
  };
...
export function loadMovieAnalysisImageAppBridge(
  projectRoot?: string
): MovieAnalysisImageAppBridge | null {
  const root = resolveProjectRoot(projectRoot);
  const abs = path.join(root, IMAGE_APP_BRIDGE_PATH);
  if (!fs.existsSync(abs)) return null;
