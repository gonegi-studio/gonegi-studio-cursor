...
import { IMAGE_APP_BRIDGE_PATH } from '../services/movieAnalysisImageAppBridge.js';
import {
  EXPECTED_ADAPTER_COUNT,
  EXPECTED_SOURCE_COUNT,
  IMAGE_APP_IMPORT_TEST_MD_PATH,
  IMAGE_APP_IMPORT_TEST_PASS_VERDICT,
  IMAGE_APP_IMPORT_TEST_REPORT_PATH,
  writeMovieAnalysisImageAppImportTestReport,
} from '../services/movieAnalysisImageAppImportTest.js';
...
console.log(`report=${IMAGE_APP_IMPORT_TEST_REPORT_PATH}`);
console.log(`markdown=${IMAGE_APP_IMPORT_TEST_MD_PATH}`);
...
if (report.final_verdict !== IMAGE_APP_IMPORT_TEST_PASS_VERDICT) {
  process.exit(1);
}
...
if (
  ...
  report.image_app_import_ready !== 'PASS' ||
  report.planning_only_status !== 'PASS'
) {
  console.error(
    'Expected source_count=4 adapter_count=24 source_count_valid=PASS adapter_count_valid=PASS all adapter imports=PASS traceability_preserved=PASS image_app_import_ready=PASS planning_only=PASS'
  );
  process.exit(1);
}
