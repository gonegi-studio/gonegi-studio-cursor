...
import {
  LEVEL2B_CONSUMPTION_CERTIFICATION_DIR,
...
import {
  ...
  LEVEL2_MASTER_CERTIFICATION_MD_PATH,
  ...
  LEVEL2_MASTER_TRACK_COUNT,
  LEVEL2_MASTER_TRACK_ENTRIES,
  writeMovieAnalysisLevel2MasterCertification,
} from '../services/movieAnalysisLevel2MasterCertification.js';
import {
  LEVEL2_RUNTIME_CERTIFICATION_DIR,
...
for (const dir of [LEVEL2_RUNTIME_CERTIFICATION_DIR, LEVEL2B_CONSUMPTION_CERTIFICATION_DIR]) {
...
for (const entry of LEVEL2_MASTER_TRACK_ENTRIES) {
...
const report = writeMovieAnalysisLevel2MasterCertification(projectRoot);

console.log(report.final_verdict);
console.log(
  `status=${report.certification_status ?? 'NONE'} source_count=${report.source_count} adapter_count=${report.adapter_count} level2_master_tracks_complete=${report.level2_master_tracks_complete} level2_runtime_complete=${report.completion_validation.level2_runtime_complete} level2b_consumption_complete=${report.completion_validation.level2b_consumption_complete} runtime_binding_complete=${report.completion_validation.runtime_binding_complete} prompt_generation_complete=${report.completion_validation.prompt_generation_complete} prompt_assembly_complete=${report.completion_validation.prompt_assembly_complete} image_runtime_ready=${report.completion_validation.image_runtime_ready} video_runtime_ready=${report.completion_validation.video_runtime_ready} image_app_consumption_ready=${report.completion_validation.image_app_consumption_ready} video_app_consumption_ready=${report.completion_validation.video_app_consumption_ready} cross_app_consumption_ready=${report.completion_validation.cross_app_consumption_ready} runtime_mapping_preserved=${report.runtime_mapping_preserved} traceability_preserved=${report.traceability_preserved} level2_master_certification_ready=${report.level2_master_certification_ready} planning_only=${report.planning_only_status}`
);
for (const audit of report.track_audits) {
  console.log(`  ${audit.track_id}: exists=${audit.report_exists} passed=${audit.track_passed}`);
}
console.log(`report=${LEVEL2_MASTER_CERTIFICATION_REPORT_PATH}`);
console.log(`markdown=${LEVEL2_MASTER_CERTIFICATION_MD_PATH}`);
...
if (report.final_verdict !== LEVEL2_MASTER_CERTIFICATION_PASS_VERDICT) {
  process.exit(1);
}
...
if (
  ...
  report.certification_status !== LEVEL2_MASTER_CERTIFICATION_STATUS_MESSAGE ||
  report.source_count !== EXPECTED_SOURCE_COUNT ||
  report.adapter_count !== EXPECTED_ADAPTER_COUNT ||
  report.level2_master_track_count !== LEVEL2_MASTER_TRACK_COUNT ||
  report.level2_master_tracks_complete !== 'PASS' ||
  ...
  report.level2_master_certification_ready !== 'PASS' ||
  report.planning_only_status !== 'PASS' ||
  report.track_audits.length !== LEVEL2_MASTER_TRACK_COUNT ||
  ...
  console.error('Expected Level 2 master certification with runtime and consumption tracks PASS');
  process.exit(1);
}

if (
  LEVEL2_RUNTIME_CERTIFICATION_STATUS_MESSAGE !== LEVEL2_MASTER_CERTIFICATION_STATUS_MESSAGE
) {
  console.error('Expected LEVEL2_COMPLETE status alignment between runtime and master certification');
  process.exit(1);
}
