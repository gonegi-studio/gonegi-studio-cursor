...
import {
  ...
  HIGH_PRIORITY_ITEM_CATEGORY_BY_BLOCKER,
  ...
  RESOLUTION_EXECUTION_TARGET_PHASE,
  SAFE_CREATE_POLICY,
...
const report = writeMvHighPriorityItemResolutionExecution(projectRoot);

console.log(report.final_verdict);
console.log(
  `status=${report.certification_status ?? 'NONE'} resolution_execution_id=${report.resolution_execution_id} source_termination_gate_ref=${report.source_termination_gate_ref} source_count=${report.source_count} adapter_count=${report.adapter_count} high_priority_item_ids=${report.high_priority_item_ids.join(',')} resolution_started=${report.resolution_started} resolved_high_priority_count=${report.resolved_high_priority_count} remaining_high_priority_count=${report.remaining_high_priority_count} next_reentry_gate_label=${report.next_reentry_gate_label} execution_scope=${report.execution_scope} termination_gate_consumed=${report.termination_gate_consumed} high_priority_item_ids_valid=${report.high_priority_item_ids_valid} high_priority_item_category_valid=${report.high_priority_item_category_valid} resolution_execution_plan_valid=${report.resolution_execution_plan_valid} resolution_status_by_item_valid=${report.resolution_status_by_item_valid} resolution_started_valid=${report.resolution_started_valid} resolution_evidence_ref_valid=${report.resolution_evidence_ref_valid} resolved_high_priority_count_valid=${report.resolved_high_priority_count_valid} remaining_high_priority_count_valid=${report.remaining_high_priority_count_valid} traceability_preserved=${report.traceability_preserved} safe_create_policy_verified=${report.safe_create_policy_verified} next_stage_ready=${report.next_stage_ready} next_stage_approved=${report.next_stage_approved} high_priority_item_ids_missing=${report.high_priority_item_ids_missing} resolution_execution_plan_missing=${report.resolution_execution_plan_missing} resolution_status_invalid=${report.resolution_status_invalid} resolution_started_invalid=${report.resolution_started_invalid} resolution_evidence_ref_missing=${report.resolution_evidence_ref_missing} high_priority_item_unresolved=${report.high_priority_item_unresolved} mv_high_priority_item_resolution_execution_ready=${report.mv_high_priority_item_resolution_execution_ready} safe_create_policy=${SAFE_CREATE_POLICY}`
);
console.log(`report=${MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_REPORT_PATH}`);
console.log(`artifact=${MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_ARTIFACT_PATH}`);
...
if (report.final_verdict !== MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_PASS_VERDICT) process.exit(1);

if (
  !fs.existsSync(path.join(projectRoot, MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_REPORT_PATH)) ||
  !fs.existsSync(path.join(projectRoot, MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_DIR)) ||
  !fs.existsSync(path.join(projectRoot, MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_EXPORT_DIR)) ||
  !fs.existsSync(path.join(projectRoot, MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_EVIDENCE_DIR)) ||
  !fs.existsSync(path.join(projectRoot, MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_MANIFEST_PATH)) ||
  !fs.existsSync(path.join(projectRoot, MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_ARTIFACT_PATH)) ||
  report.source_count !== EXPECTED_SOURCE_COUNT ||
  report.adapter_count !== EXPECTED_ADAPTER_COUNT ||
  report.execution_checks.length !== 8 ||
  ...
  EXPECTED_HIGH_PRIORITY_BLOCKER_CODES.every(
    (blockerCode) => report.resolution_status_by_item[blockerCode] === RESOLUTION_STATUS_RESOLVED
  ) === false ||
  ...
  report.termination_gate_consumed !== 'PASS' ||
  report.high_priority_item_ids_valid !== 'PASS' ||
  report.high_priority_item_category_valid !== 'PASS' ||
  report.resolution_execution_plan_valid !== 'PASS' ||
  ...
  report.resolution_execution_plan_missing !== false ||
  report.resolution_status_invalid !== false ||
  report.resolution_started_invalid !== false ||
  report.resolution_evidence_ref_missing !== false ||
  report.high_priority_item_unresolved !== false ||
  report.mv_high_priority_item_resolution_execution_ready !== 'PASS' ||
  report.certification_status !== HIGH_PRIORITY_RESOLUTION_EXECUTION_COMPLETED_STATUS ||
  report.next_stage_approved !== true ||
  report.execution_checks.every((check) => check.status === 'PASS') === false
) {
  console.error(
    'Expected PASS with HIGH_PRIORITY_RESOLUTION_EXECUTION_COMPLETED, resolution_started=true, and 3 items RESOLVED'
  );
  process.exit(1);
}

const artifact = JSON.parse(
  fs.readFileSync(path.join(projectRoot, MV_HIGH_PRIORITY_ITEM_RESOLUTION_EXECUTION_ARTIFACT_PATH), 'utf8')
) as {
  high_priority_item_ids: string[];
  high_priority_item_category: Record<string, string>;
  resolution_execution_plan: Record<string, { target_phase: string; category: string }>;
...
if (
  ...
  EXPECTED_HIGH_PRIORITY_BLOCKER_CODES.every(
    (blockerCode) => artifact.high_priority_item_category[blockerCode] === HIGH_PRIORITY_ITEM_CATEGORY_BY_BLOCKER[blockerCode]
  ) === false ||
  EXPECTED_HIGH_PRIORITY_BLOCKER_CODES.every(
    (blockerCode) => artifact.resolution_execution_plan[blockerCode]?.target_phase === RESOLUTION_EXECUTION_TARGET_PHASE
  ) === false ||
  EXPECTED_HIGH_PRIORITY_BLOCKER_CODES.every(
    (blockerCode) => artifact.resolution_status_by_item[blockerCode] === RESOLUTION_STATUS_RESOLVED
  ) === false ||
  ...
  artifact.resolution_execution_complete !== true ||
  artifact.next_stage_ready !== true
) {
  console.error('Artifact fields do not match expected high priority item resolution execution output');
  process.exit(1);
}

for (const blockerCode of EXPECTED_HIGH_PRIORITY_BLOCKER_CODES) {
  const evidencePath = path.join(projectRoot, artifact.resolution_evidence_ref[blockerCode]);
  if (!fs.existsSync(evidencePath)) {
    console.error(`Missing resolution evidence file for ${blockerCode}`);
    process.exit(1);
  }
}
