...
/**
 * PHASE-33C: Controlled-generation refiner with immutable Gonegi/Dana name tokens.
 */
export async function refineControlledPrompt(
    controlledPrompt: string,
    bridgedContext?: string
): Promise<string> {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const model = MODELS.DIRECTOR_MODEL;

    const systemInstruction = `
        You refine SingleCanvas controlled-generation scene prompts for Studio Ghibli cel-animation.
        RULES:
        - NEVER rename Gonegi or Dana. Keep exact spelling "Gonegi" and "Dana".
        - Do NOT replace character names with generic nouns (boy, girl, protagonist, companion, lead).
        - Do NOT rewrite identity law or firewall blocks if present in context.
        - Output ONLY the refined scene action text (no markdown).
    `;
    ...
    const response = await ai.models.generateContent({
        ...
        contents: {
            parts: [
                {
                    text: `Refine this controlled prompt while preserving locked character names.
CONTEXT:\n${bridgedContext ?? '(none)'}\n\nPROMPT:\n${controlledPrompt}`,
                },
            ],
        },
    ...
    const responseText = await getResponseText(response);
    const refined = (responseText || controlledPrompt).trim();
    return preserveLockedCharacterNames(refined, controlledPrompt);
}
